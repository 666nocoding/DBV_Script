...
type settings struct {
	...
	fail        *deque.Deque[string]
}

var s settings = settings{
	urls: new(deque.Deque[string]),
	fail: new(deque.Deque[string]),
}

func (s *settings) LoadUrlFile(urlfile string) error {
...
func getArgs(ctx context.Context, cmd *cli.Command) error {
	...
	s.maxgor = int(cmd.Int32("maxgor"))
	for i := range cmd.Args().Len() {
		s.urls.PushBack(cmd.Args().Get(i))
	}
	if cmd.String("file") != "" {
		if err := s.LoadUrlFile(cmd.String("file")); err != nil {
